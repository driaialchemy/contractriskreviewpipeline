...
def test_full_pipeline_produces_three_traces(mock_contract_loader: Any) -> None:
    ...
    with patch(
        "src.agents.extraction.get_dataset_zip_path",
        return_value=PROJECT_ROOT / "tests" / "dummy_dataset.zip",
    ):
        result = asyncio.run(run_pipeline(state))

    assert len(result.execution_traces) == 3
    assert [trace.sequence_number for trace in result.execution_traces] == [1, 2, 3]
    assert result.execution_traces[0].step_name == "extraction"
    assert result.execution_traces[0].duration_ms >= 0
